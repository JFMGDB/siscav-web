...
- [ ] **MON-01**: Falhas em `getLastCapture` / cadeia de logs em `src/lib/api/monitor.ts` são **distinguíveis** de “sem dados” na UI (mensagem ou estado de erro)
- [ ] **MON-02**: Campo `confidence` em capturas não é **inventado** (0,95 fixo); vem da API ou é removido/ocultado até o backend fornecer
