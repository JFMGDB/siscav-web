...
describe("validateCameraPreviewUrl", () => {
  ...
  it("accepts https when page is https", () => {
    const r = validateCameraPreviewUrl("https://cam.example/stream.mjpg", {
      pageIsHttps: true,
    });
  ...
  it("blocks http when page is https (mixed content)", () => {
    const r = validateCameraPreviewUrl("http://192.168.1.10/video.mjpg", {
      pageIsHttps: true,
    });
    expect(r.ok).toBe(false);
  ...
  it("rejects javascript:", () => {
    const r = validateCameraPreviewUrl("javascript:alert(1)", {
      pageIsHttps: false,
    });
    expect(r.ok).toBe(false);
  ...
  it("marks m3u8 as HLS", () => {
    const r = validateCameraPreviewUrl(
...
describe("redactUrlForDisplay", () => {
  it("removes userinfo", () => {
    expect(redactUrlForDisplay("https://user:secret@host/path")).toBe(
      "https://host/path",
    );
  });
});
