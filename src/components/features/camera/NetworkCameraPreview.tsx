...
export default function NetworkCameraPreview() {
  ...
  const [syncedNetworkUrl, setSyncedNetworkUrl] = useState(config.networkUrl);
  const [active, setActive] = useState<{ href: string; isHls: boolean } | null>(
    null,
  );
  ...
  if (config.networkUrl !== syncedNetworkUrl) {
    setSyncedNetworkUrl(config.networkUrl);
