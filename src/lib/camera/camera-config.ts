...
export function loadCameraConfig(): CameraPersistedConfig {
    ...
    return parseCameraConfig(window.localStorage.getItem(CAMERA_CONFIG_STORAGE_KEY));
}

export function saveCameraConfig(config: CameraPersistedConfig): void {
    if (typeof window === 'undefined') return;
    window.localStorage.setItem(CAMERA_CONFIG_STORAGE_KEY, JSON.stringify(config));
